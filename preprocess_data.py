...
def process_mesh(mesh_filepath, target_filepath, executable, additional_args):
    ...
    command = ['python', 'sample_sdf.py', '-m', mesh_filepath, '-o', target_filepath] + additional_args
...
if __name__ == "__main__":
    ...
    if args.surface_sampling:
        executable = os.path.join(deepsdf_dir, "dataprocessing/bin/SampleVisibleMeshSurface")
        subdir = ws.surface_samples_subdir
        extension = ".ply"
    else:
        executable = os.path.join(deepsdf_dir, "dataprocessing/bin/PreprocessMesh")
            ...
            try:
                ...
                meshes_targets_and_specific_args.append(
                    (
                        ...
                        mesh_filename,   # fix in 2023.03.02, for mesh_filename already include shape_dir
                        processed_filepath,
                        specific_args,
                    )
                )
